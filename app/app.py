...
def search_results():
    ...
    if search_query:
        ...
        if len(suggestions) == 0:
            suggestions = []
        if len(tweets) == 0:
            return render_template('pages/search.html', search_query = search_query, error = "No tweets Found! Please try different set of keywords.", tweets = None, suggestions=suggestions, countries=countries)
