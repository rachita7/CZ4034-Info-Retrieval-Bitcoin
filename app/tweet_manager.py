...
class TweetManager:
    solr = pysolr.Solr('http://localhost:8983/solr/tweets_data_updated', always_commit=True)
    # solr.optimize()
    ...
    def get_tweets_by_exact_match(query, countries = []):
        ...
        try:
            if "content:" in query or "date:" in query or "country:" in query:
                ...
                respon = TweetManager.solr.search(f"username:{query}",fq = f"country:{countries}", rows=100)
        except:
    ...
    def spell_check(query):
        try:
            url = 'http://localhost:8983/solr/tweets_data_updated/spell?spellcheck.q='+query+'&spellcheck=true'
    ...
    def rank_by_most_relevant_tweets(query, tweets):
        ...
        try:
            ...
            logging.error("Error in getting most relevant tweet!")
            return []
    ...
    @staticmethod
    def rank_by_positive(tweets):
        return sorted(tweets, key= lambda x: x['inference'][0], reverse=True)
